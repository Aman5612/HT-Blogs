...
export class MainContentComponent implements OnInit {
  ...
  private fetchPackages(): void {
    ...
    const fetchPromises = this.packageIds.map(id => 
      this.http.get<any>(`https://staging.holidaytribe.com:3000/package/getPackageByIds/${id}`)
        .toPromise()
        .then(response => {
          if (response && response.status && response.result && response.result[0]) {
  ...
  private processContent(): void {
    ...
    this.sanitizedContent = this.sanitizer.bypassSecurityTrustHtml(modifiedContent);
  }
